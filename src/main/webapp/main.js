...
var Container = React.createClass({
    ...
    upload: function (event) {
        ...
        up.onSuccess = function () {
            if (this.xhr.status === 200) {
                ...
                that.setState({
                    status: "success", msg: "Success", files: json
                });
            } else {
                console.log("upload success, ret not ok", this);
                that.setState({
                    status: "danger", msg: this.xhr.response, files: {
                    }
                });
            }
        };
        this.setState({
            ok: true, msg: "Uploading, please wait...", files: {
            }
        });
        up.upload(event);
    },
    render: function () {
        ...
        React.DOM.h2(null, "Dublin Core to MARC 21 transformer"),
        React.DOM.p(null, "This web service allows you to convert a DC Simple metadata file to the Marc21 format."),
        React.DOM.hr(null),
        React.DOM.pre(null, "With stylesheet retrieved from http://www.loc.gov/standards/marcxml/xslt/DC2MARC21slim.xsl."),
        React.DOM.hr(null),
        FileUploadBox({
            onUpload: this.upload
        }),
        StatusBox({
            status: this.state.status, text: this.state.msg, progress: this.state.progress
        }),
        DownloadBox({
        ...
        React.DOM.pre(null, "$ curl -d @input.dc.xml -o output.marc.xml http://weblicht.sfs.uni-tuebingen.de/converter/DC2Marc/rest"))))));
    }
});
...
var FileUploadBox = React.createClass({
    ...
    render: function () {
        var specialAddon = {
            marginRight: "10px", border: "none", background: "none"
        };
        return (
        React.DOM.div({
        ...
        "Upload your Dublin Core file:"),
        React.DOM.span({
            className: "btn btn-primary btn-file"
        },
        "Browse",
        React.DOM.input({
            type: "file", name: "fileUpload", onChange: this.onUpload
        })))));
    }
});
...
var StatusBox = React.createClass({
    ...
    render: function () {
        ...
        return React.DOM.div({
            className: "statusBox", style: style
        },
        React.DOM.div({
...
var DownloadBox = React.createClass({
    ...
    render: function () {
        ...
        var header = $.isEmptyObject(files) ? (React.DOM.span(null)): (React.DOM.p(null, "Download generated MARC 21: "));
        return (React.DOM.div(null, " ", header,
        React.DOM.ul({
            className: "downloadBox list-group"
        },
        " ", files, " ")));
    }
});
